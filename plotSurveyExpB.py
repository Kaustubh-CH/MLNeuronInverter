...
def get_parser():
    ...
    parser.add_argument("--formatName",  default='expB.8kHz', help="data name extesion ")
...
def print_exp_summary(bigD,expMD):
    ...
    waves=bigD['waveform']
    stims=bigD['stim']
    sweepCnt=bigD['sweepCnt']
    print('Exp-summary:',expMD['shortName'])
    for ia,ampl in enumerate(ampls):        
        print('ia=%2d stimAmpl=%.2f numSweep=%d'%(ia,ampl,sweepCnt[ia]))
